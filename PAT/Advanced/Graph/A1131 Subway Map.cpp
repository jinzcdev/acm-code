...
int main() {
    ...
    while (k--) {
        ...
        dfs(st, 0);
        printf("%d\n", minCntStop);
        int preline = line[path[0] * 10000 + path[1]], transferStop = path[0];
        for (int i = 1; i < path.size(); i++) {
            int nowline = line[path[i - 1] * 10000 + path[i]];
            if (nowline != preline) {
                printf("Take Line#%d from %04d to %04d.\n", preline, transferStop, path[i - 1]);
                preline = nowline;
                transferStop = path[i - 1];
            }
        }
        printf("Take Line#%d from %04d to %04d.\n", preline, transferStop, dest);
    }
    return 0;
}