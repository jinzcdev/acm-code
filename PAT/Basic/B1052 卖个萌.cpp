...
int main(){
    ...
    for (int i = 0; i < 3; i++){
        ...
        for (int j = 0; j < len; j++){
            ...
            if (str[j] == ']'){
                right = j - 1;
                if (i == 0){
                    strncpy(hand[cnt++], str + left, right - left + 1);
                }else if (i == 1){
                    strncpy(eye[cnt++], str + left, right - left + 1);
                }else{
                    strncpy(mouth[cnt++], str + left, right - left + 1);
                }
                num[i]++;
            }
        }
    ...
    while (k--){
        ...
        for (int i = 0; i < 5; i++){
            scanf("%d", &ans[i]);
            int j = i > 2 ? 5 - i - 1 : i;
            if (ans[i] > num[j] || ans[i] < 1)  // 输入的序号大于最大值或者小于1, 则为错误
                flag = false;
        }
        if (!flag)
